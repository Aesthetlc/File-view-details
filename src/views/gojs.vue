<template>
  <div ref="myDiagramDiv" id="myDiagramDiv" style="min-width:900px; height:600px"></div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.myDiagram.nodeTemplate = gm(
      ...
      // 定义节点的文本
      gm(go.TextBlock, { margin: 10, height: 40, font: 'bold 11px Helvetica, bold Arial, sans-serif' }, new go.Binding('text', 'key')),
      gm(
        go.Panel,
        {
    ...
    // 连接线样式
    this.myDiagram.linkTemplate = gm(go.Link, { routing: go.Link.Orthogonal, corner: 5 }, gm(go.Shape, { strokeWidth: 3, stroke: '#555' }))
    this.init()
  },
  methods: {
    ...
    init() {
      ...
      // 子组件中心位置数据
      this.ipData = this.diagramData[0].key
